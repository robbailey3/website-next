...
describe('[COMPONENT]: Button', () => {
  let wrapper: ShallowWrapper<typeof IconButton>;
  const createWrapper = (props: Partial<IconButtonProps>) => {
    const defaultProps = {
      type: 'button',
      variant: 'primary',
      size: 'medium',
    ...
    wrapper = shallow(<IconButton {...{ ...defaultProps, ...props }} />);
  };
  ...
  describe('[PROP]: variant', () => {
    test.each([
      ...
      ({ variant, expectedClassName }) => {
        createWrapper({ variant } as IconButtonProps);
  ...
  describe('[PROP]: size', () => {
    test.each([
      {
        size: 'small',
        expectedClassName: 'p-1 text-sm',
      },
      {
        size: 'medium',
        expectedClassName: 'p-2 text-base',
      },
      {
        size: 'large',
        expectedClassName: 'p-4 text-lg',
      },
    ])(
  ...
  describe('[PROP]: label', () => {
    it('should render a button with children="Hello" when children="Hello" is passed', () => {
      createWrapper({ label: 'Hello' });
      expect(wrapper.find('button').render().text()).toBe('Hello');
    });
  });
});
